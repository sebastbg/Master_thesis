module crystal_plasticity
    ...
    !                   12,  '{110} <-111>'  '{211} <-111>'
    !                   13,  '{110} <-111>'                   '{321} <-111>'
    !                   123, '{110} <-111>'  '{211} <-111>'   '{321} <-111>'

        real(kind=8)    :: n(48,3), b(48,3), schmid(3,3,48), schmid_sym(3,3,48),&
                           schmid_skw(3,3,48), schmid_sym_vec(5,48), schmid_skw_vec(3,48), tmp(6)
        integer         :: i, cs, s = 0
        real(kind=8), parameter     ::  inv2 = 1.d0/sqrt(2.d0),  &
                                        inv6 = 1.d0/sqrt(6.d0),  &
        ...
        n(1:12,:)  = n(1:12,:)*inv2
        n(13:24,:) = n(13:24,:)*inv6
        n(25:48,:) = n(25:48,:)*inv14
        b = b *(1.d0/sqrt(3.d0))

        ! for hvert plan og slip
        do i=1, 48
            schmid(:,:,i)       = outer3(b(i,:), n(i,:))
        ...
        case (12)
            ...
            allocate(P(5,Nslips), Omega(3,Nslips), STAT=s)
            P(:,:12)       =  schmid_sym_vec(:,:12)
            P(:,13:24)     =  schmid_sym_vec(:,13:24)
            P(:,25:36)     = -schmid_sym_vec(:,:12)
            P(:,37:48)     = -schmid_sym_vec(:,13:24)
            Omega(:,:12)   =  schmid_skw_vec(:,:12)
            Omega(:,13:24) =  schmid_skw_vec(:,13:24)
            Omega(:,25:36) = -schmid_skw_vec(:,:12)
            Omega(:,37:48) = -schmid_skw_vec(:,13:24)

        case (13)
            Nslips = 72
            allocate(P(5,Nslips), Omega(3,Nslips), STAT=s)
            P(:,:12)       =  schmid_sym_vec(:,:12)
            P(:,13:36)     =  schmid_sym_vec(:,25:)
            P(:,37:48)     = -schmid_sym_vec(:,:12)
            P(:,49:72)     = -schmid_sym_vec(:,25:)
            Omega(:,:12)   =  schmid_skw_vec(:,:12)
            Omega(:,13:36) =  schmid_skw_vec(:,25:)
            Omega(:,37:48) = -schmid_skw_vec(:,:12)
            Omega(:,49:72) = -schmid_skw_vec(:,25:)
        ...
        case (123)
            ...
            P(:,:48)       =  schmid_sym_vec
            P(:,49:)       = -schmid_sym_vec
            Omega(:,:48)   =  schmid_skw_vec
            Omega(:,49:)   = -schmid_skw_vec
    ...
    subroutine init_crss(cs, crss, scale1, scale2)
    ! Scaling of the CRSS for different slips systems
      use globals, only: Nslips
      !input
      integer         :: cs
      real(8)         :: initial, scale1, scale2
      real(8), allocatable  ::  crss(:)

      intent (in)     :: cs, scale1, scale2
      !Output
      intent (inout)  :: crss
      ...
      case (12)
          ...
          crss(25:36)   = initial
          crss(37:48)   = initial *scale1

      case (13)
          allocate(crss(Nslips))
          crss(:12)     = initial
          crss(13:36)   = initial * scale2
          crss(37:48)   = initial
          crss(49:72)   = initial * scale2
