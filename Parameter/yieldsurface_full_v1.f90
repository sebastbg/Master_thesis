program yield_surface_full
    ...
    real(KIND =8) :: stress(6), dstrain(6), dt, tol, strainrate(3,3), T_list(1000,6), T11, T22, T33, &
                    ...
                    cs1, cs2, cs3, tmp_out, T12, T13,  T23, T(3,3), input(6), out(100), gm_temp(96)
    ...
    params(4) = 2           ! Exponent in the RSCYF - Regularized Single Crystal Yield Function
    ...
    call linspace(0.010d0,5.d0,array)
    ...
    do a1=1,100! !Alpha 1 loop
        ...
        do a2=1,100! !alpha2 loop
            ...
            do n=1,1000!10 
                ...
                !Saving largest gmdot array value
                !if(maxval(gmdot) .GT. maxval(gm_temp)) gm_temp = gmdot
                
                !Saving all gmdot arrays. total strain map
                gm_sum = gmdot + gm_sum
            end do

            ! Convert the results into fractions of eache system
            !gm_temp = gm_temp / sum(gm_temp)
            gm_temp = gm_sum/sum(gm_sum)
            ...
            do i =1,12
                cs1 = cs1 + gm_temp(i) + gm_temp(i+48)
            end do
            
            do i =13,24
                cs2 = cs2 + gm_temp(i) + gm_temp(i+48)
            end do
    
            do i =25,48
                cs3 = cs3 + gm_temp(i) + gm_temp(i+48)
            end do
 
            !write(*,*) cs1, cs2, cs3

            tmp_out = 0.d0
            if (cs1 .GT. 0.1d0) then
                tmp_out = tmp_out + 4.d0
            end if

            if (cs2 .GT. 0.1d0) then
                tmp_out = tmp_out + 5.d0
            end if

            if (cs3 .GT. 0.1d0) then
                tmp_out = tmp_out + 6.d0 
            end if
