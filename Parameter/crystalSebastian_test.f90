module crystal_plasticity
    ...
    subroutine init_bcc_slipsystems(cs)
    ! Subroutine defines slip systems, build Schmid matrix,its symmetric part "P"
    ! and skew part "Omega", both global variables defined in module "globals"
    !
    ! Inputs: cs    -   specifies the set of slip systems used for the calculations as:
    ...
    !                   123, '{110} <-111>'  '{211} <-111>'   '{321} <-111>'
        ...
        real(8), allocatable:: n(:,:), b(:,:), schmid(:,:,:), schmid_sym(:,:,:), schmid_skw(:,:,:), &
                               schmid_sym_vec(:,:), schmid_skw_vec(:,:), tmp(:)
        integer             :: i, cs, s = 0
        real(kind=8), parameter     ::  inv3 = 1.d0/sqrt(3.d0),  &
                                        inv2 = 1.d0/sqrt(2.d0),  &
        ...
        case (13)
            ...
            allocate(P(5,Nslips), Omega(3,Nslips), STAT=s)
            P(:,:12)       =  schmid_sym_vec(:,:12)
            P(:,13:36)     =  schmid_sym_vec(:,24:)
            P(:,37:48)     = -schmid_sym_vec(:,:12)
            P(:,49:72)     = -schmid_sym_vec(:,24:)
            Omega(:,:12)   =  schmid_skw_vec(:,:12)
            Omega(:,13:36) =  schmid_skw_vec(:,24:)
            Omega(:,37:48) = -schmid_skw_vec(:,:12)
            Omega(:,49:72) = -schmid_skw_vec(:,24:)
    ...
    end subroutine init_bcc_slipsystems

    subroutine initial_crss(cs, crss)
      use globals, only: Nslips
      !input
      integer         :: cs
      real(8)         :: initial, scale
      ...
      case (1)
          ...
          crss(:12)  = initial
          crss(13:)  = initial * scale

      case (12)
          allocate(crss(Nslips))
          crss(:12)     = initial
          crss(13:18)   = initial * scale
          crss(19:30)   = initial
          crss(31:36)   = initial * scale
      ...
      case (1234)
          ...
          crss(31:36)   = initial * scale
          crss(37:48)   = initial
          crss(49:60)   = initial * scale
          crss(61:72)   = initial


    end subroutine initial_crss
