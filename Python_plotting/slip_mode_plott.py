...
for n in range(0,1):
    for m in range (0,1):
        for i in range (0,3):
            ...
            x_res = np.zeros(48)
            for t in range (0,47):
                x_res[t] =abs( temp[t]) + abs(temp[48+t])
        ...
        rects2 = ax.bar(x, x_2, width/3, label=r'$n=50$', edgecolor='grey', color=[(0.9882352941176471, 0.5529411764705883, 0.3843137254901961)])
        rects3 = ax.bar(x + width/3, x_3, width/3, label=r'$n=100$', edgecolor='grey', color=[(0.5529411764705883, 0.6274509803921569, 0.796078431372549)],rasterized=True)
